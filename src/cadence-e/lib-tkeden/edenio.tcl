...
# [Ben] Only use focusfollowsmouse on Unix, as on Win32 this is non-standard
#       behaviour and will confuse users
# [Ash] Also Mac (although it doesn't seem to make much difference)
...
if {$_tkeden_variant == "dtkeden"} {
    ...
    # tkeden
    set variantversion "Cadence-DE $_tkeden_version"
}
...
if {$_tkeden_variant == "dtkeden"} {
	...
	button .radios.send -text "Send" -underline 0 \
	    ...
	    set text [.p.inputframe.text get 1.0 end]
...
## Create a paned window for the input and console output
panedwindow .p -orient vertical
frame .p.inputframe
frame .p.outframe

## Create a text box for the input
text .p.inputframe.text -width 80 -height 15 -yscrollcommand ".p.inputframe.scroll set" \
...
.p.inputframe.text configure -tabs [font measure [.p.inputframe.text cget -font] 00]
scrollbar .p.inputframe.scroll -command ".p.inputframe.text yview"

pack .p.inputframe.scroll -side right -fill y
pack .p.inputframe.text -side right -fill both -expand 1

##proc toggleorientation w {
##   $w config -ori [expr {[$w cget -ori] eq "horizontal"? "vert": "hori"}]
##}

pack .p -fill both -expand 1
.p add .p.inputframe -height 200
.p add .p.outframe -height 100

## Create a text box for console output
text .p.outframe.outtext
.p.outframe.outtext configure -foreground "green" -background "black" \
	-width 80 -height 5 -yscrollcommand ".p.outframe.scroll set" \
	-font edencode

scrollbar .p.outframe.scroll -command ".p.outframe.outtext yview"
pack .p.outframe.scroll -side right -fill y
pack .p.outframe.outtext -side right -fill both -expand 1

## Make the console text box uneditable (but still copyable)
foreach event {<KeyPress> <<PasteSelection>>} {
   bind .p.outframe.outtext $event break
}
bind .p.outframe.outtext <Control-c> {event generate %W <<Copy>>}

## Add a window for text output also
toplevel .outwin
wm title .outwin "$variantversion: Output"
wm withdraw .outwin
text .outwin.text -foreground "green" -background "black" \
	-yscrollcommand ".outwin.scroll set" -font edencode
scrollbar .outwin.scroll -command ".outwin.text yview"
pack .outwin.scroll -side right -fill y
pack .outwin.text -side right -fill both -expand 1

## Toggle between separate window and integrated window
proc toggleoutputwindow {} {
   if {[winfo ismapped .outwin]} {
      wm withdraw .outwin
	  .p add .p.outframe -height 100
   } else {
      wm deiconify .outwin
...
## Called from EDEN to output text
proc outputMessage args {
   foreach arg $args {
      .p.outframe.outtext insert end $arg
      .p.outframe.outtext yview moveto 1.0
      .outwin.text insert end $arg
      .outwin.text yview moveto 1.0
   }
}
...
if {$_tkeden_apple} {
	...
	$m add command -label "Cut" \
		...
		-command {event generate .p.inputframe.text <<Cut>>}
	$m add command -label "Copy" \
		-accelerator "Command-C" \
		-command {event generate .p.inputframe.text <<Copy>>}
	$m add command -label "Paste" \
		-accelerator "Command-V" \
		-command {event generate .p.inputframe.text <<Paste>>}
	$m add command -label "Select all" \
		-accelerator "Command-A" \
		-command {.p.inputframe.text tag add sel 1.0 end}
#	$m add command -label "Select none" \
#		-accelerator "Control-\\" \
#		-command {.p.inputframe.text tag add sel}
#		-command {.p.inputframe.text tag delete sel}
#		-command {event generate .p.inputframe.text <Control-backslash>}
	$m add separator
	...
	$m add command -label "Select all" -underline 7 \
		-accelerator "Control-/" \
		-command {event generate .p.inputframe.text <Control-slash>}
	$m add command -label "Select none" \
		-accelerator "Control-\\" \
		-command {event generate .p.inputframe.text <Control-backslash>}
...
$m add separator
$m add checkbutton -label "Output in separate window" -variable showoutputwindow -command {toggleoutputwindow}
...
if {$_tkeden_apple} {
	...
	bind all <Command-Q> { exit }
	bind .p.inputframe.text <Command-a> { .p.inputframe.text tag add sel 1.0 end }
	bind .p.inputframe.text <Command-A> { .p.inputframe.text tag add sel 1.0 end }
	bind .p.inputframe.text <Command-Up> { previous }
	bind .p.inputframe.text <Command-Down> { next }
	bind .p.inputframe.text <Control-u> { controlU }
	bind .p.inputframe.text <Control-U> { controlU }
	bind .p.inputframe.text <Command-Left> { .p.inputframe.text mark set insert {insert linestart} }
	bind .p.inputframe.text <Command-Right> { .p.inputframe.text mark set insert {insert lineend} }

	# on the Mac, Command-A is Select All as standard, and I don't want
	# to conflict with that, so using this for Accept instead...
	bind .p.inputframe.text <Command-Return> {
		accept;
	...
	bind .p.inputframe.text <Alt-a> { accept }
	bind .p.inputframe.text <Alt-A> { accept }
	bind .p.inputframe.text <Control-Alt-Up> { previous }
	bind .p.inputframe.text <Meta-Up> { previous }
	bind .p.inputframe.text <Alt-p> { previous }
	bind .p.inputframe.text <Alt-P> { previous }
	bind .p.inputframe.text <Control-Alt-Down> { next }
	bind .p.inputframe.text <Meta-Down> { next }
	bind .p.inputframe.text <Alt-n> { next }
	bind .p.inputframe.text <Alt-N> { next }
	bind .p.inputframe.text <Control-Alt-0> { clearInputWindow }
	bind .p.inputframe.text <Meta-0> { clearInputWindow }
	bind .p.inputframe.text <Control-u> { controlU }
	bind .p.inputframe.text <Control-U> { controlU }
}
...
bindtags .p.inputframe.text {all .p.inputframe.text Text}
...
proc previous {} {
    ...
    .p.inputframe.text insert end [lindex $pentries $ppos]
}
...
proc clearInputWindow {} {
    .p.inputframe.text delete 1.0 end
}


proc controlU {} {
    # delete the text to the left of the cursor
    .p.inputframe.text delete {insert linestart} insert
}
...
proc include style {
	...
	if {$style == "Open"} {
		...
		while {[gets $incFile line] >= 0} {
		    .p.inputframe.text insert end "$line\n"
		}
		.p.inputframe.text see end
		close $incFile
	    } else {
